...
from std_msgs.msg import Int16   # ✅ import Int16 message

def run(imu_sensor, yaw_pub):
    ...
    # ✅ publish yaw in degrees as Int16
    yaw_msg = Int16()
    yaw_msg.data = int(yaw_degrees)   # convert to integer
    yaw_pub.publish(yaw_msg)

    # log full orientation
...
def main():
    ...
    # ✅ Publisher for yaw (degrees) as Int16
    yaw_pub = rospy.Publisher("/yaw", Int16, queue_size=10)

    rate = rospy.Rate(75) 
    while not rospy.is_shutdown():
        run(imu_sensor, yaw_pub)
        rate.sleep()
