...
def movement():
    ...
    # If obstacle ahead
    if forward_distance < 30 and moved:
        turn_angle(math.pi/2, cmd) # turn left for now, untill we have cv readings
    else:
        moved = 1
        cmd.linear.x = min(0.1, (forward_distance-5) * 0.01)
        cmd.angular.z = 0
        pub.publish(cmd)
...
def turn_angle(rotation_angle, cmd):
    ...
    target_yaw = snap_angle(target_yaw)
    rate = rospy.Rate(30)
...
def snap_angle(angle_rad):
    """Snap an angle in degrees to the closest of [0, 90, -90, 180]."""
    angle_deg = math.degrees(angle_rad)
    targets = [0, 90, -90, 180]
    value = min(targets, key=lambda x: abs(angle_deg - x))
